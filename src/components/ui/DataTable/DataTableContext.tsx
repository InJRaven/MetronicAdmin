...
const DataTableContext = createContext<IDataTableContextProps<any> | undefined>(
  undefined
);
...
const DataTableProvider = <TData extends object>(
  ...
  const defaultValues: Partial<TDataTableProps<TData>> = {
    messages: {
  ...
  const fetchServerSideData = useCallback(async () => {
    ...
    try {
      const requestParams: TDataTableRequestParams = {
